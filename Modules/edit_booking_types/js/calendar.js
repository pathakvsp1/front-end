$(document).ready(function() {
	...
	//alert(staffid);
    $.ajax({
        url: "/staff/staffSchedule/getDateOverrides",
        type: "GET",
        data: { staffId: staffid,scheduleId:scheduleId }
    }).done(function(dateOverrides) {
		        $.ajax({
            url: "/staff/staffSchedule/getWeeklyAvailabilities",
            type: "GET",
            data: { staffId: staffid,scheduleId:scheduleId}
...
var calendar = {
    ...
    submit: async (dateOverrides) => {
        ...
        if (dateOverrides.dates.length > 0) {
            ...
            for (var i = 0; i < dateOverrides.dates.length; i++) {
                var submitObj = { staffId: staffid };
                submitObj.scheduleId = scheduleid;
                submitObj.staffId = staffid;
                ...
                if (_this.dateOverridesFetched.findIndex(dateOverwrite => dateOverwrite.date == submitObj.date) >= 0) {
                    ...
                    await $.ajax({
                        url: "/staff/staffSchedule/updateDateOverrides",
                        method: "POST",
                        data: submitObj
                    });
                } else {
                    ...
                    await $.ajax({
                        url: "/staff/staffSchedule/addDateOverrides",
                        method: "POST",
                        data: submitObj
                    }).done(function(msg) {
                        console.log(msg)
    ...
    deleteDateOverride: async function(overrideId) {
        ...
        _this = this;
        $.ajax({
            url: "/staff/staffSchedule/deleteDateOverride",
            method: "POST",
